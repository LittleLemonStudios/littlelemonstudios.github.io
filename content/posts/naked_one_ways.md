...
{{< 
  pixel_art 
  src="/gif/one_way/working.gif" 
  scale="three"
  caption="By disabling the platform until the player is above the platform (rather than based on the player's velocity, we find something much cleaner for the player)"
>}}
...
{{< 
  pixel_art 
  src="/gif/one_way/snapping.gif" 
  scale="three"
  caption="It's exaggerated here for the blog, but we can now snap the player to the platform if their jump will just miss the platform"
>}}
...
func _physics_process(_delta: float) -> void:
	...
	# Compute the x-diff to ensure the player is over the platform
	# for edge clipping
	var x_diff = player.global_position.x - global_position.x
	var above_platform = x_diff < 5 and x_diff > -10
	...
	if collision_shape_2d.disabled and above_platform and player.velocity.y >= 0 and y_diff < 2.0:
